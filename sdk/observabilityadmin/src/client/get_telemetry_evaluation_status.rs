...
impl super::Client {
    ...
    ///   - [`home_region(Option<String>)`](crate::operation::get_telemetry_evaluation_status::GetTelemetryEvaluationStatusOutput::home_region): <p>The Amazon Web Services Region that is designated as the home region for multi-region telemetry evaluation. The home region is the single management point for all multi-region operations on this account. This field is only present when multi-region telemetry evaluation is active.</p>
    ///   - [`region_statuses(Option<Vec::<RegionStatus>>)`](crate::operation::get_telemetry_evaluation_status::GetTelemetryEvaluationStatusOutput::region_statuses): <p>A list of per-region telemetry evaluation statuses. Each entry indicates the evaluation status for a specific spoke region included in the multi-region configuration. This field is only present when multi-region telemetry evaluation is active.</p>
