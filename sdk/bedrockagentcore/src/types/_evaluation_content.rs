...
impl EvaluationContent {
    #[allow(irrefutable_let_patterns)]
    /// Tries to convert the enum instance into [`Text`](crate::types::EvaluationContent::Text), extracting the inner [`String`](::std::string::String).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_text(&self) -> ::std::result::Result<&::std::string::String, &Self> {
        if let EvaluationContent::Text(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`Text`](crate::types::EvaluationContent::Text).
