// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_allow_vended_log_delivery_for_resource_input_input(
    object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    input: &crate::operation::allow_vended_log_delivery_for_resource::AllowVendedLogDeliveryForResourceInput,
) -> ::std::result::Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    if let Some(var_1) = &input.delivery_source_arn {
        object.key("deliverySourceArn").string(var_1.as_str());
    }
    if let Some(var_2) = &input.log_type {
        object.key("logType").string(var_2.as_str());
    }
    if let Some(var_3) = &input.resource_arn_being_authorized {
        object.key("resourceArnBeingAuthorized").string(var_3.as_str());
    }
    Ok(())
}
